...
class home extends FE_Controller {
	...
	function category_alias($alias='',$page=1){
		$perpage = 18;
		$limit = ($page-1) * $perpage;
		$this->_get_new_product();
		$this->_get_hot_product();
		$this->_get_discount_product();
		$cat = $this->category_model->getCategoryByAlias($alias);
		if($cat){
			$cat->title = $cat->aTitle;
			$this->assigns->cat = $cat;
			$catid = $cat->cat_id;
			$this->assigns->site = array(
				'title'=>$cat->title[$this->assigns->lang],
	...
	function newsdetail_alias($alias=''){
		$this->_get_new_product();
		$this->_get_hot_product();
		$this->_get_discount_product();
	...
	function product_detail($id=''){
		...
		if($product){
			...
			$this->_addView('lang_product','product_',$product->product_id);
		}
