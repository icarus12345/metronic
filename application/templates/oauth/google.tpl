...
<style>
  ...
  <div id="gConnect" >
    <div onClick="signInClick()"
        alt="Sign in with Google+">Google +</div>
  </div>
...
var helper = (function() {
  ...
  return {
    ...
    disconnectServer: function() {
      ...
      $.ajax({
        type: 'POST',
        url: $(location).attr('origin') + '/oauth/google/disconnect',
        async: false,
    ...
    connectServer: function(code) {
      ...
      $.ajax({
        type: 'POST',
        url: $(location).attr('origin') + '/oauth/google/connect?state={{ STATE }}',
    ...
    activities: function(success, failure) {
      ...
      $.ajax({
        type: 'GET',
        url: $(location).attr('origin') + '/oauth/google/activities',
        contentType: 'application/octet-stream; charset=utf-8',
        success: success,
        error: failure,
        processData: false
...
$(document).ready(function() {
  ...
  if ($('[data-clientid="691346579027-erj1al3tavepadjv38bsot6pq1cuthge.apps.googleusercontent.com"]').length > 0) {
