  ...
  * @return mixed */
    ...
    protected function normalizeFilename($filename) {
        if (isset($this->config['filenameChangeChars']) &&
            ...
            //
        return $this->convertUrl($filename);
    }
    

    protected function convertUrl($str) {
        $remove = "~ ` ! @ # $ % ^ & * ( ) _ + | \\ = ' \" ” “ ; : ? / > , < ] [ { }";
    ...
    protected function normalizeDirname($dirname) {
        ...
        return $this->convertUrl($dirname);
    }
    ...
    protected function checkUploadedFile(array $aFile=null) {
        ...
        foreach ($_CONFIG['types'] as $key => $value) {
            if($config['uploadDir'].'/'.$key == $this->typeDir){
                if( $value['upload_max_filesize'] > 0 ){
                    $upload_max_filesize = $value['upload_max_filesize'];
                }
                break;
            }
        }
		...
		if($filesize > $upload_max_filesize){
			return $this->label("(".(int)($filesize/1024)."KB) - File size must be less than $upload_max_filesize_mb MB.");
		}
