    ...
    if (file.hasClass('selected') && files.length && (files.length > 1)) {
        ...
        if (this.access.files['delete'])
            html += '<a href="kcact:rm"' + (!data.writable ? ' class="denied"' : '') + '>' +
                    this.label("Delete") + '</a>';
        if (this.access.files['delete'])
            html += '<a href="kcact:ed"' + (!data.writable ? ' class="edit"' : '') + '>' +
                    "Edit" + '</a>';
        html += '</div>';
        ...
        $('.menu a[href="kcact:ed"]').click(function() {
            if (!data.writable)
                return false;
            browser.hideDialog();
            var file = {dir: browser.dir, file: data.name};
            window.open('/libraries/kcfinder/jcrop/crop.php?dir='+browser.dir+'&file='+data.name+'', 'kcfinder_textbox',
